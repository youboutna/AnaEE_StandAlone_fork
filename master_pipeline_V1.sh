...
# $5 DATAB_BASE { [postgresql] - mysql } 
    ...
    for entry in `ls -d data/yedGen/*/`; do
    
        if [[ "$entry" != */connexion/ ]] ; then
        
          rm data/yedGen/*.*
          rm data/ontop/*.*
          rm data/corese/*.* && rm data/corese/*
                    
          cp data/yedGen/connexion/connexion.graphml data/yedGen/
          cp -R $entry/*.*  data/yedGen
